...
class Init():
    ...
    def do_setting_activemq_log4j(self, loglevel):
        ...
        self.replace_all(ACTIVEMQ_CONF + "/log4j.properties", "log4j\.logger\.org\.apache\.activemq\.audit=[^,]+",
                         "log4j.logger.org.apache.activemq.audit=" + loglevel)

    def do_setting_activemq_main(self, name, messageLimit, storageUsage, tempUsage, maxConnection, frameSize, topics,
                                 queues, enabledScheduler, enabledAuth):
        ...
        self.replace_all(ACTIVEMQ_CONF + "/activemq.xml", '<constantPendingMessageLimitStrategy limit="\d+"/>',
                         '<constantPendingMessageLimitStrategy limit="' + str(messageLimit) + '"/>')
        self.replace_all(ACTIVEMQ_CONF + "/activemq.xml", '<storeUsage limit="[^"]+"/>',
                         '<storeUsage limit="' + storageUsage + '"/>')
        self.replace_all(ACTIVEMQ_CONF + "/activemq.xml", '<tempUsage limit="[^"]+"/>',
                         '<tempUsage limit="' + tempUsage + '"/>')
        self.replace_all(ACTIVEMQ_CONF + "/activemq.xml", '\?maximumConnections=1000',
                         "?maximumConnections=" + str(maxConnection))
        self.replace_all(ACTIVEMQ_CONF + "/activemq.xml", 'wireFormat\.maxFrameSize=104857600',
                         "wireFormat.maxFrameSize=" + str(frameSize))
...
<plugins>
    ...
    def do_setting_activemq_wrapper(self, minMemoryInMB, maxMemoryInMb):
        ...
        self.replace_all(ACTIVEMQ_HOME + "/bin/linux-x86-64/wrapper.conf", "#?wrapper\.java\.initmemory=\d+",
                         'wrapper.java.initmemory=' + str(minMemoryInMB))
        self.replace_all(ACTIVEMQ_HOME + "/bin/linux-x86-64/wrapper.conf", "#?wrapper\.java\.maxmemory=\d+",
                         'wrapper.java.maxmemory=' + str(maxMemoryInMb))
    ...
    def do_init_activemq(self):
        ...
        self.replace_all(ACTIVEMQ_HOME + "/bin/linux-x86-64/wrapper.conf",
                         "set\.default\.ACTIVEMQ_DATA=%ACTIVEMQ_BASE%\/data",
                         "set.default.ACTIVEMQ_DATA=/data/activemq")

        # Fix bug #4 "Cannot mount a custom activemq.xml"
        self.replace_all(ACTIVEMQ_HOME + "/bin/linux-x86-64/wrapper.conf",
                         "set\.default\.ACTIVEMQ_CONF=%ACTIVEMQ_BASE%/conf$",
                         "set.default.ACTIVEMQ_CONF=%ACTIVEMQ_BASE%/conf.tmp")
    ...
    def setting_all(self):
        ...
        self.do_setting_activemq_main(os.getenv('ACTIVEMQ_NAME', 'localhost'),
                                      ...
                                      os.getenv('ACTIVEMQ_TEMP_USAGE', '20 gb'),
